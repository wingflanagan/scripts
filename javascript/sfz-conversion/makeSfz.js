...
function processGroup(params) {
  ...
  if (group.type === "POST-LEGATO" || group.type === "VELOCITY_POST-LEGATO") {
    ...
    if (headerProps.ampeg_release === undefined) headerProps.ampeg_release = 1;
...
function main() {
  ...
  // Group any ROUND-ROBIN groups together (both velocity and non-velocity)
  const roundRobinGroupMap = {};
  const nonRoundRobinGroups = [];

  definition.noteFiles.groups.forEach(group => {
    if (group.type.indexOf("ROUND-ROBIN") >= 0) {
      // Create a key based on header properties that must be common.
      // For non-velocity groups these values might be undefined.
      const key = `${group.trigger}|${group.group}|${group.loop_mode}|${group.lovel || ""}|${group.hivel || ""}|${group.volume || ""}|${group.crossfadeOverlap || ""}`;
      if (!roundRobinGroupMap[key]) {
        roundRobinGroupMap[key] = [];
      }
      roundRobinGroupMap[key].push(group);
    } else {
      nonRoundRobinGroups.push(group);
    }
  });

  // Process non-round-robin groups normally.
  nonRoundRobinGroups.forEach(group => {
    ...
    if (group.type.indexOf("VELOCITY_") === 0 &&
        group.type.indexOf("LEGATO") < 0 &&
        group.type.indexOf("RELEASE") < 0) {
      velocityGroups.push(section);
    }
  });

  // Process round-robin groups: for each common key, merge the round-robin layers.
  for (let key in roundRobinGroupMap) {
    const groupsArray = roundRobinGroupMap[key];
    // Use the first group as the base for header properties.
    let baseSection = processGroup({
      group: groupsArray[0],
      instrumentRange: fullRange,
      files: inputFiles,
      instrumentName
    });
    // Clear out regions so we can merge round-robin layers.
    baseSection.regions = [];
    // Process each round-robin layer and add a seq_position.
    groupsArray.forEach((grp, idx) => {
      const section = processGroup({
        ...
        files: inputFiles,
        instrumentName,
        extra: { roundRobin: { rrRegex: grp.fileRegex, seq_position: idx + 1 } }
      });
      // Append regions from this round-robin layer.
      baseSection.regions = baseSection.regions.concat(section.regions);
    });
    // Set the total number of round-robin layers.
    baseSection.headerProps.seq_length = groupsArray.length;
    sections.push(baseSection);
    // If the base group is a velocity type, add it to velocityGroups for crossfade processing.
    if (groupsArray[0].type.indexOf("VELOCITY_") === 0) {
